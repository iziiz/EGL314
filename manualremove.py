...
class PhotoBoothPython:
    ...
    def capture_clean_background_reference(self, num_frames=10, delay_ms=50):
        frames = []
        ...
        if not frames:
            print("[X] Failed to capture reference frames.")
            return

        # Use median to reduce movement/noise artifacts
        stacked = np.stack(frames, axis=0)
        median_frame = np.median(stacked, axis=0).astype(np.uint8)

        self.reference_image = Image.fromarray(median_frame).convert("RGBA")
        self.reference_image.save("reference_background_clean.png")
        print("[✓] Saved clean averaged background as 'reference_background_clean.png'")
    ...
    def setup_control_buttons(self):
        ...
        ttk.Button(ctl, text="Capture BG Reference (B)", command=self.capture_clean_background_reference).grid(row=0, column=2, padx=5)

    def setup_shortcuts(self):
        self.root.bind("<space>", lambda _e: self.start_countdown())
        self.root.bind("<Escape>", lambda _e: self.on_close())
        self.root.bind("<b>", lambda _e: self.capture_clean_background_reference())
    ...
    def remove_background(self):
        if self.original_image is None or self.background_reference is None:
            return

        # Resize reference image to match input frame
        reference_resized = cv2.resize(self.background_reference, (self.original_image.shape[1], self.original_image.shape[0]))

        # Compute absolute difference between color images
        diff = cv2.absdiff(self.original_image, reference_resized)

        # Convert to grayscale and threshold to create mask
        diff_gray = cv2.cvtColor(diff, cv2.COLOR_BGR2GRAY)
        _, mask = cv2.threshold(diff_gray, 40, 255, cv2.THRESH_BINARY)  # Threshold can be tuned (40–60)
        ...
        # Create alpha matte
        fg_np = cv2.cvtColor(self.original_image, cv2.COLOR_BGR2BGRA)
        fg_np[..., 3] = np.clip(mask, 0, 255).astype(np.uint8)

        # Save the result
        os.makedirs("cutout", exist_ok=True)
        cutout_path = os.path.join("cutout", "cutout.png")
        cv2.imwrite(cutout_path, fg_np)

        # Update preview
        cutout_pil = Image.fromarray(cv2.cvtColor(fg_np, cv2.COLOR_BGRA2RGBA))
        cutout_tk = ImageTk.PhotoImage(cutout_pil.resize((640, 480)))
        self.cutout_label.config(image=cutout_tk)
        self.cutout_label.image = cutout_tk
        self.cutout_image_path = cutout_path
        self.show_mode("cutout")
    ...
    def show_background_thumbnails(self, no_bg_img, input_path):
        ...
        def on_bg_click(bg_img):
            # Compose selected background with cutout
            composited = Image.alpha_composite(bg_img, no_bg_img)
            self.display_image(composited)
            self.last_composited = composited
            self.last_composited_path = input_path.replace(".png", "_edited.png")
            self.show_preview_controls()

        # For each background image, create composite thumbnail with the cutout
            for idx, bg_path in enumerate(self.background_images):
                bg_img = Image.open(resource_path(bg_path)).convert("RGBA").resize(no_bg_img.size)
                thumb_img = Image.alpha_composite(bg_img, no_bg_img).copy()
                thumb_img.thumbnail((200, 150), Image.LANCZOS)

                imgtk = ImageTk.PhotoImage(thumb_img)
                self.thumb_imgs.append(imgtk)

                btn = tk.Button(thumbs_container, image=imgtk, command=lambda bg=bg_img: on_bg_click(bg))
